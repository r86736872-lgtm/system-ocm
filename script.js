...
function moveToNextContract() {
  ...
  let isEmergency = typeof activeDay === "string" && activeDay.startsWith("emg-");
    ...
    if (
      ...
      if (isEmergency) {
        ...
        if (!item.taskType || item.taskType === "daily") {
          ...
          let itemDailyIndex = dailySpecificData.findIndex(d => d.originalIndex === nextIndex);
          
          // Izinkan jika index masih masuk dalam porsi tab hari ini
          if (itemDailyIndex >= allowedStartIndex && itemDailyIndex <= allowedEndIndex) {
